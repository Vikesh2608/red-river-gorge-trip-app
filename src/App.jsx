...
{tab==='settlement' && <div style={card}><h2>💸 Smart Final Settlement</h2><p>Option C: easiest final payments. Just pay the person shown below.</p><div style={{display:'grid',gap:12}}>{settlements.length?settlements.map((s,i)=><div key={i} style={{padding:16,background:'#fff',borderRadius:18,border:'1px solid #e5e7eb'}}><div style={{fontWeight:900,fontSize:18}}>{s.from} ➜ {s.to}</div><div style={{marginTop:6,color:'#b91c1c'}}>Pay ${s.amount.toFixed(2)}</div><div style={{display:'flex',gap:8,marginTop:10,flexWrap:'wrap'}}><input style={{...input,minWidth:120}} placeholder='Paid amount' value={settlementPaid[i]||''} onChange={e=>setSettlementPaid({...settlementPaid,[i]:e.target.value})}/><button style={btn} onClick={()=>setSettlementPaid({...settlementPaid,[i]:s.amount.toFixed(2)})}>Mark Cleared</button></div><div style={{marginTop:8,fontSize:14,color:Number(settlementPaid[i]||0)>=s.amount?'#15803d':'#475569'}}>{Number(settlementPaid[i]||0)>=s.amount?'Cleared ✅':`Remaining $${Math.max(0,s.amount-Number(settlementPaid[i]||0)).toFixed(2)}`}</div></div>):<div style={{padding:16,background:'#ecfeff',borderRadius:16}}>All balances cleared ✅</div>}</div><div style={{marginTop:14,padding:14,background:'#eff6ff',borderRadius:16}}>Example: Instead of everyone paying everyone, only the above payments are needed.</div></div>}
