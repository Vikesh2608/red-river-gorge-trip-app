...
const settlements = useMemo(()=>{const creditors=[]; const debtors=[]; Object.entries(balances).forEach(([name,val])=>{if(val>0.01)creditors.push({name,amt:val}); else if(val<-0.01)debtors.push({name,amt:-val});}); const tx=[]; let i=0,j=0; while(i<debtors.length && j<creditors.length){const pay=Math.min(debtors[i].amt,creditors[j].amt); tx.push(`${debtors[i].name} pays ${creditors[j].name} $${pay.toFixed(2)}`); debtors[i].amt-=pay; creditors[j].amt-=pay; if(debtors[i].amt<0.01)i++; if(creditors[j].amt<0.01)j++; } return tx;},[balances]);
