...
const [arrivalStatus,setArrivalStatus]=useState([{name:'Amit',status:'On the way',eta:'1:30 PM'},{name:'John',status:'Reached',eta:'Arrived'}]);
const [arrName,setArrName]=useState('');
const [arrState,setArrState]=useState('On the way');
const [arrEta,setArrEta]=useState('');
const [liveTrackers,setLiveTrackers]=useState([{name:'Amit',lat:'37.79',lng:'-83.68',status:'Near Slade'},{name:'John',lat:'37.80',lng:'-83.70',status:'10 mins away'}]);
...
const loadRealtime = async()=>{
 ...
 if(data){ if(data.members) setMembers(data.members); if(data.expenses) setExpenses(data.expenses); if(data.publicPosts) setPublicPosts(data.publicPosts); if(data.comments) setComments(data.comments); if(data.liveTrackers) setLiveTrackers(data.liveTrackers); }
};
loadRealtime();
...
const addExpense=()=>{if(expenseTitle&&expenseAmount){const people=participants.length?participants:members.map(m=>m.name);setExpenses([...expenses,{title:expenseTitle,amount:Number(expenseAmount),paidBy:paidBy||'Unknown',people}]);setExpenseTitle('');setExpenseAmount('');setPaidBy('');setParticipants([]);}};
...
{tab==='budget' && <div style={card}><h2 style={{fontSize:32,margin:'0 0 8px 0'}}>💰 Total Budget: ${totalBudget}</h2><h3 style={{color:'#2563eb',marginTop:0}}>Default Split: ${split}</h3><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><select style={input} value={expenseTitle} onChange={e=>setExpenseTitle(e.target.value)}><option value=''>Category</option><option>Stay</option><option>Food</option><option>Activities</option><option>Fuel</option><option>Other</option></select><input style={input} placeholder='Amount' value={expenseAmount} onChange={e=>setExpenseAmount(e.target.value)}/><select style={input} value={paidBy} onChange={e=>setPaidBy(e.target.value)}><option value=''>Paid by</option>{members.map((m,i)=><option key={i} value={m.name}>{m.name}</option>)}</select><div style={{width:'100%',padding:12,border:'1px solid #dbeafe',borderRadius:14,background:'#fff'}}><b>Select Participants</b><div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:8}}>{members.map((m,i)=><label key={i} style={{fontSize:14}}><input type='checkbox' checked={participants.includes(m.name)} onChange={(e)=>setParticipants(e.target.checked?[...participants,m.name]:participants.filter(n=>n!==m.name))}/> {m.name}</label>)}</div></div><button style={btn} onClick={addExpense}>Add</button></div><div style={{marginTop:16,padding:16,background:'#eff6ff',borderRadius:16}}><b>Who Owes / Who Gets Back</b>{Object.entries(balances).map(([n,v])=><div key={n} style={{color:v>=0?'green':'#b91c1c'}}>{n}: {v>=0?`gets $${v.toFixed(2)}`:`owes $${Math.abs(v).toFixed(2)}`}</div>)}<hr style={{margin:'12px 0'}}/><b>Settle Up</b>{settlements.length?<div style={{marginTop:10,overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',background:'#fff'}}><thead><tr style={{background:'#dbeafe'}}><th style={{padding:10,textAlign:'left'}}>Who Owes</th><th style={{padding:10,textAlign:'left'}}>Pay To</th><th style={{padding:10,textAlign:'left'}}>Amount</th></tr></thead><tbody>{settlements.map((s,i)=><tr key={i} style={{borderTop:'1px solid #e5e7eb'}}><td style={{padding:10}}>{s.from}</td><td style={{padding:10,color:'#15803d',fontWeight:700}}>{s.to}</td><td style={{padding:10,fontWeight:700}}>${s.amount.toFixed(2)}</td></tr>)}</tbody></table></div>:<div>Everyone is settled up ✅</div>}</div><ul>{expenses.map((e,i)=><li key={i} style={{margin:'10px 0'}}>{e.title} - ${e.amount} • Paid by {e.paidBy} • Split among {(e.people||[]).join(', ')} <button onClick={()=>removeExpense(i)}>Delete</button></li>)}</ul></div>}
