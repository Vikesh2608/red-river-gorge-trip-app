...
{tab==='people' && <div style={card}><h2>👥 Travelers</h2><div style={{marginBottom:14}}><a href='https://chat.whatsapp.com/I9s6EZK81X05llarQSPyDs' target='_blank' style={{...btn,textDecoration:'none',display:'inline-block'}}>💬 Open WhatsApp Group</a><button style={btn} onClick={()=>window.open('https://faq.whatsapp.com/5913398998672934','_blank')}>📍 Share Live Location Help</button></div><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><input style={input} placeholder='Name' value={name} onChange={e=>setName(e.target.value)}/><input style={input} placeholder='Phone Number' value={phone} onChange={e=>setPhone(e.target.value)}/><button style={btn} onClick={addMember}>{memberSaving?'Saving...':(editingMember!==null?'Update':'Add')}</button></div><ul>{members.map((m,i)=><li key={i} style={{margin:'10px 0'}}><b>{m.name}</b> • 📞 <a href={`tel:${m.phone}`} style={{textDecoration:'none',fontSize:20,marginRight:8}}>📲</a> <a href={`https://wa.me/${String(m.phone).replace(/\D/g,'')}`} target='_blank' style={{textDecoration:'none',fontSize:20,marginRight:8}}>💬</a> {m.phone||'N/A'} <button onClick={()=>editMember(i)}>Edit</button> <button onClick={()=>removeMember(i)}>Delete</button></li>)}</ul></div>}
