...
const TABLES={members:'trip_members',expenses:'trip_expenses',comments:'trip_comments',posts:'trip_posts',trackers:'trip_trackers',settlements:'trip_settlements'};
...
const loadRealtime = async()=>{
 const [{data:m},{data:e},{data:c},{data:p},{data:t},{data:s}] = await Promise.all([
  ...
  supabase.from(TABLES.trackers).select('*').order('updated_at',{ascending:false}),
  supabase.from(TABLES.settlements).select('*').order('id')
 ]);
 setMembers(m||[]); setExpenses(e||[]); setComments(c||[]); setPublicPosts(p||[]); setLiveTrackers(t||[]); setSettlementRows(s||[]);
...
.on('postgres_changes',{event:'*',schema:'public',table:TABLES.trackers},()=>loadRealtime())
.on('postgres_changes',{event:'*',schema:'public',table:TABLES.settlements},()=>loadRealtime())
.subscribe();
...
const settlements = useMemo(()=>{const debtors=[]; const creditors=[]; Object.entries(balances).forEach(([name,val])=>{if(val<-0.01) debtors.push({name,amount:-val}); if(val>0.01) creditors.push({name,amount:val});}); const out=[]; let i=0,j=0; while(i<debtors.length && j<creditors.length){const pay=Math.min(debtors[i].amount,creditors[j].amount); out.push({from:debtors[i].name,to:creditors[j].name,amount:pay}); debtors[i].amount-=pay; creditors[j].amount-=pay; if(debtors[i].amount<0.01)i++; if(creditors[j].amount<0.01)j++; } return out;},[balances]);
...
try{
 if(editingMember!==null){
   const target=members[editingMember]; const oldName=target.name;
   const {error}=await supabase.from(TABLES.members).update({name:name.trim(),phone}).eq('id',target.id);
   if(error) throw error;
   setMembers(prev=>prev.map((m,i)=>i===editingMember?{...m,name:name.trim(),phone}:m)); await syncRenameEverywhere(oldName,name.trim());
...
const syncRenameEverywhere=async(oldName,newName)=>{try{
if(!oldName||!newName||oldName===newName) return;
// merge duplicate member names firstsetExpenses(prev=>prev.map(e=>({...e,paidby:e.paidby===oldName?newName:e.paidby,people:(e.people||[]).map(p=>p===oldName?newName:p)})));setComments(prev=>prev.map(c=>({...c,name:c.name===oldName?newName:c.name})));setPublicPosts(prev=>prev.map(p=>({...p,name:p.name===oldName?newName:p.name})));await supabase.from(TABLES.expenses).update({paidby:newName}).eq('paidby',oldName);
const {data:expRows}=await supabase.from(TABLES.expenses).select('*');
for(const row of (expRows||[])){
 const arr=(row.people||[]).map(p=>p===oldName?newName:p);
 if(JSON.stringify(arr)!==JSON.stringify(row.people||[])) await supabase.from(TABLES.expenses).update({people:arr}).eq('id',row.id);
}await supabase.from(TABLES.comments).update({name:newName}).eq('name',oldName);await supabase.from(TABLES.posts).update({name:newName}).eq('name',oldName);
const {data:allMembers}=await supabase.from(TABLES.members).select('*');
const dupes=(allMembers||[]).filter(m=>m.name===newName);
if(dupes.length>1){
 const keep=dupes[0].id;
 for(const d of dupes.slice(1)){ await supabase.from(TABLES.members).delete().eq('id',d.id); }
}
const {data:freshMembers}=await supabase.from(TABLES.members).select('*').order('id'); if(freshMembers) setMembers(freshMembers);}catch(e){}};
const startEditExpense=(i)=>{const e=expenses[i];setEditingExpense(i);setExpenseTitle(e.title||'');setExpenseSubType(e.subtype||'');setExpenseAmount(String(e.amount||''));setPaidBy(e.paidby||'');setParticipants(e.people||[]);};
...
try {
 ...
 if(editingExpense!==null){ const target=expenses[editingExpense]; const {error}=await supabase.from(TABLES.expenses).update({title:expenseTitle,subtype:expenseSubType||'',amount:Number(expenseAmount),paidby,people}).eq('id',target.id); if(error) throw error; } else { const { error } = await supabase.from(TABLES.expenses).insert({
  title: expenseTitle,
  ...
  const { data } = await supabase.from(TABLES.expenses).select('*').order('id').select('*').order('id');
 setExpenses(data || []);
 setExpenseTitle('');
 setExpenseSubType('');
 setExpenseAmount('');
...
const addComment=async()=>{if(commentText.trim()){if(editingComment&&comments[editingComment]?.id){await supabase.from(TABLES.comments).update({name:commentName||'Guest',text:commentText}).eq('id',comments[editingComment].id);setEditingComment(null);}else{await supabase.from(TABLES.comments).insert({name:commentName||'Guest',text:commentText,reply_to:replyTo?comments[replyTo]?.id:null});}setReplyTo(null);setCommentName('');setCommentText('');setTypingUsers({});}};
const removeComment=async(i)=>{if(comments[i]?.id){await supabase.from(TABLES.comments).delete().eq('id',comments[i].id);}else{setComments(comments.filter((_,x)=>x!==i));}};
const addPost=async()=>{if(postText.trim()){await supabase.from(TABLES.posts).insert({name:postName||'Guest',text:postText,likes:0});setPostName('');setPostText('');}};
const likePost=async(i)=>{const p=publicPosts[i]; if(!p?.id) return; await supabase.from(TABLES.posts).update({likes:(p.likes||0)+1}).eq('id',p.id);};
const removePost=async(i)=>{const p=publicPosts[i]; if(p?.id){await supabase.from(TABLES.posts).delete().eq('id',p.id);} else {setPublicPosts(publicPosts.filter((_,x)=>x!==i));}};
...
const addLiveTracker=async()=>{if(trackName && location.lat){await supabase.from(TABLES.trackers).upsert({name:trackName,lat:Number(location.lat).toFixed(6),lng:Number(location.lng).toFixed(6),status:'Live now',updated_at:new Date().toISOString()},{onConflict:'name'});setTrackName('');}};
const removeLiveTracker=async(i)=>{if(liveTrackers[i]?.id){await supabase.from(TABLES.trackers).delete().eq('id',liveTrackers[i].id);}else{setLiveTrackers(liveTrackers.filter((_,x)=>x!==i));}};
useEffect(()=>{let watchId=null; if(trackingOn && navigator.geolocation){watchId=navigator.geolocation.watchPosition(async(pos)=>{setLocation({lat:pos.coords.latitude,lng:pos.coords.longitude,status:'GPS Live'}); if(trackName){await supabase.from(TABLES.trackers).upsert({name:trackName,lat:pos.coords.latitude.toFixed(6),lng:pos.coords.longitude.toFixed(6),status:'Moving',updated_at:new Date().toISOString()},{onConflict:'name'});} },()=>{}, {enableHighAccuracy:true,maximumAge:5000,timeout:10000});} return ()=>{if(watchId!==null) navigator.geolocation.clearWatch(watchId);};},[trackingOn,trackName]);
const startVoice=()=>{
 ...
 const smartReply=(q)=>{
   ...
   if(text.includes('where is')||text.includes('track')) return '📍 Open Tracker tab to view live traveler locations.'; return `🌐 Smart Assistant for ${place}: ask travel, routes, food, weather, tracker, finance, news, science, sports, or directions.`;
 };
...
<div style={{display:'flex',gap:10,overflowX:'auto',paddingBottom:8,margin:'18px 0'}}>{['people','budget','settlement','stay','cars','timeline','arrival','tracker','comments','guide','assistant','emergency'].map(t=><button key={t} style={{...btn,whiteSpace:'nowrap',opacity:tab===t?1:0.72,transform:tab===t?'scale(1.04) translateY(-2px)':'scale(1)',transition:'all .22s ease'}} onClick={()=>setTab(t)}>{t}</button>)}</div>

{tab==='people' && <div style={card}><h2>👥 Travelers</h2><div style={{marginBottom:14,display:'flex',gap:10,flexWrap:'wrap'}}><a href='https://chat.whatsapp.com/I9s6EZK81X05llarQSPyDs' target='_blank' style={{...btn,textDecoration:'none',display:'inline-block'}}>💬 Open WhatsApp Group</a><button style={btn} onClick={()=>(()=>{const msg=location.lat?`📍 My Live GPS: https://maps.google.com/?q=${location.lat},${location.lng}`:'📍 Share your live location from WhatsApp: Attach > Location > Live Location';window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`,'_blank');})()}>📍 WhatsApp GPS Share</button></div><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><input style={input} placeholder='Choose traveler' value={name} onChange={e=>setName(e.target.value)}/><input style={input} placeholder='Phone Number' value={phone} onChange={e=>setPhone(e.target.value)}/><button style={btn} onClick={addMember}>{memberSaving?'Saving...':(editingMember!==null?'Update':'Add')}</button></div><ul>{members.map((m,i)=><li key={i} style={{margin:'10px 0'}}><b>{m.name}</b> • 📞 <a href={`tel:${m.phone}`} style={{textDecoration:'none',fontSize:20,marginRight:8}}>📲</a> <a href={`https://wa.me/${String(m.phone).replace(/\D/g,'')}`} target='_blank' style={{textDecoration:'none',fontSize:20,marginRight:8}}>💬</a> {m.phone||'N/A'} <button onClick={()=>editMember(i)}>Edit</button> <button onClick={()=>removeMember(i)}>Delete</button></li>)}</ul></div>}

{tab==='budget' && <div style={card}><h2 style={{fontSize:32,margin:'0 0 8px 0'}}>💰 Total Budget: ${totalBudget}</h2><h3 style={{color:'#2563eb',marginTop:0}}>Crystal Clear Split View</h3><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:12,marginBottom:14}}><div style={{padding:14,background:'#ecfeff',borderRadius:16}}><b>🏡 Stay Total</b><div>${stayExpenses.reduce((a,b)=>a+Number(b.amount||0),0).toFixed(2)}</div></div><div style={{padding:14,background:'#f0fdf4',borderRadius:16}}><b>🎯 Activities Total</b><div>${activityExpenses.reduce((a,b)=>a+Number(b.amount||0),0).toFixed(2)}</div></div><div style={{padding:14,background:'#fefce8',borderRadius:16}}><b>🧾 Other Total</b><div>${otherExpenses.reduce((a,b)=>a+Number(b.amount||0),0).toFixed(2)}</div></div></div><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><select style={input} value={expenseTitle} onChange={e=>setExpenseTitle(e.target.value)}><option value=''>Category</option><option>Stay</option><option>Food</option><option>Activities</option><option>Fuel</option><option>Other</option></select>{expenseTitle==='Activities' && <select style={input} value={expenseSubType} onChange={e=>setExpenseSubType(e.target.value)}><option value=''>Activity Name</option><option>Boating</option><option>Zip Line</option><option>Kayaking</option><option>Horse Riding</option><option>Hiking Gear</option><option>Other Activity</option></select>}{expenseTitle==='Food' && <select style={input} value={expenseSubType} onChange={e=>setExpenseSubType(e.target.value)}><option value=''>Food Type</option><option>Breakfast</option><option>Lunch</option><option>Dinner</option><option>Snacks</option><option>Drinks</option><option>Groceries</option><option>BBQ</option></select>}<input style={input} placeholder='Amount' value={expenseAmount} onChange={e=>setExpenseAmount(e.target.value)}/><select style={input} value={paidby} onChange={e=>setPaidBy(e.target.value)}><option value=''>Paid by</option>{members.map((m,i)=><option key={i} value={m.name}>{m.name}</option>)}</select><div style={{width:'100%',padding:12,border:'1px solid #dbeafe',borderRadius:14,background:'#fff'}}><b>Select Participants</b><div style={{display:'flex',gap:14,flexWrap:'wrap',marginTop:8}}><label style={{fontSize:14,fontWeight:700}}><input type='checkbox' checked={participants.length===members.length && members.length>0} onChange={(e)=>setParticipants(e.target.checked?members.map(m=>m.name):[])}/> All Participants</label>{members.map((m,i)=><label key={i} style={{fontSize:14}}><input type='checkbox' checked={participants.includes(m.name)} onChange={(e)=>setParticipants(e.target.checked?[...participants,m.name]:participants.filter(n=>n!==m.name))}/> {m.name}</label>)}</div></div><button style={btn} onClick={addExpense}>{editingExpense!==null?'Update':'Add'}</button>{editingExpense!==null?<button style={{...btn,background:'linear-gradient(135deg,#64748b,#334155)'}} onClick={()=>{setEditingExpense(null);setExpenseTitle('');setExpenseSubType('');setExpenseAmount('');setPaidBy('');setParticipants([]);}}>Cancel</button>:null}</div><div style={{marginTop:16,padding:16,background:'#eff6ff',borderRadius:16}}><b>Category Wise Clear Ledger</b>{categoryLedgers.map((g,i)=><div key={i} style={{marginTop:14,padding:14,background:'#fff',borderRadius:14,border:'1px solid #dbeafe'}}><div style={{fontWeight:800,fontSize:18}}>{g.title}{g.subtype?` (${g.subtype})`:''}</div><div style={{marginTop:6}}>Paid by <b style={{color:'#15803d'}}>{g.paidby}</b> • Total ${g.amount.toFixed(2)} • Each Share ${g.share.toFixed(2)}</div><div style={{marginTop:8,fontSize:14,color:'#334155'}}>Participants: {g.participants.join(', ')}</div><table style={{width:'100%',marginTop:10,borderCollapse:'collapse'}}><thead><tr><th style={{textAlign:'left',padding:8}}>Who Owes</th><th style={{textAlign:'left',padding:8}}>Pay To</th><th style={{textAlign:'left',padding:8}}>Amount</th></tr></thead><tbody>{g.owes.map((o,k)=><tr key={k} style={{borderTop:'1px solid #e5e7eb'}}><td style={{padding:8}}>{o.name}</td><td style={{padding:8,color:'#15803d'}}>{g.paidby}</td><td style={{padding:8,fontWeight:700}}>${o.amount.toFixed(2)}</td></tr>)}</tbody></table></div>)}<div style={{padding:12,background:'#ecfeff',borderRadius:14,margin:'14px 0'}}><b>Crystal Separate Transactions</b><div>Each category above is kept separate. No combining between payers.</div></div><div style={{padding:12,background:'#ecfeff',borderRadius:14,marginBottom:10}}><b>Quick Summary</b><div>Each category above already shows exactly who owes whom separately.</div><div>Use Stay / Activities / Food ledgers for direct payments.</div></div><b>Settlement moved to separate tab</b><div style={{marginTop:8}}>Use the Settlement tab beside Budget for payment tracking.</div></div><div style={{marginTop:14,fontWeight:800}}>Expense History</div><ul>{expenses.map((e,i)=><li key={i} style={{margin:'10px 0'}}>{e.title}{e.subtype?` (${e.subtype})`:''} - ${e.amount} • Paid by {e.paidby} • Split among {(e.people||[]).join(', ')} <button onClick={()=>startEditExpense(i)}>Edit</button> <button onClick={()=>removeExpense(i)}>Delete</button></li>)}</ul></div>}

{tab==='settlement' && <div style={card}><h2>💸 Separate Category Settlements</h2><p>No combining between people. Each payer gets reimbursed separately by category. Use the cards below for exact balances and payment tracking.</p><div style={{padding:12,background:'#eef2ff',borderRadius:14,marginBottom:12}}><b>📊 Accountant Mode Summary</b><div style={{marginTop:6}}>Cross-person netting disabled for clarity.</div><div style={{marginTop:6}}>Use the settlement cards below for exact category balances, partial payments, remaining balances, and settled status.</div></div><div style={{display:'flex',gap:10,flexWrap:'wrap',marginBottom:12}}><button style={btn} onClick={()=>{document.title='Red River Gorge Trip - Settlement Report';window.print();}}>📄 Download PDF</button><button style={{...btn,background:'linear-gradient(135deg,#059669,#0f766e)'}} onClick={()=>{const w=window.open('','_blank');const rows=categoryLedgers.map(g=>`<div style="margin:18px 0;padding:16px;border:1px solid #ddd;border-radius:14px"><h3>${g.title}${g.subtype?` (${g.subtype})`:''}</h3><p><b>Paid by:</b> ${g.paidby} | <b>Total:</b> $${g.amount.toFixed(2)}</p>${g.owes.map(o=>`<div>${o.name} pays ${g.paidby} <b>$${o.amount.toFixed(2)}</b></div>`).join('')}</div>`).join('');w.document.write(`<html><head><title>RRG Finance Report</title></head><body style="font-family:Arial;padding:30px"><h1>🏕️ Red River Gorge Trip Finance Report</h1><p>Date: ${new Date().toLocaleString()}</p><p>Total Budget: $${totalBudget.toFixed(2)} | Travelers: ${members.length}</p>${rows}<hr/><p>Generated by Red River Gorge Trip App</p></body></html>`);w.document.close();w.print();}}>✨ Pro PDF</button><button style={btn} onClick={async()=>{const text=categoryLedgers.map(g=>`${g.title}${g.subtype?` (${g.subtype})`:''}
${g.owes.map(o=>`${o.name} pays ${g.paidby} $${o.amount.toFixed(2)}`).join('\n')}`).join('\n\n'); if(navigator.share){try{await navigator.share({title:'Trip Settlement',text});}catch(e){}} else {window.open(`https://wa.me/?text=${encodeURIComponent(text)}`,'_blank');}}}>💬 Share WhatsApp</button><button style={btn} onClick={async()=>{const text=categoryLedgers.map(g=>`${g.title}: `+g.owes.map(o=>`${o.name}->${g.paidby} $${o.amount.toFixed(2)}`).join(', ')).join(' | '); await navigator.clipboard.writeText(text); setAiTip('📋 Settlement copied to clipboard');}}>📋 Copy Summary</button></div><div style={{display:'grid',gap:14}}>{categoryLedgers.map((g,i)=><div key={i} style={{padding:16,background:'#fff',borderRadius:18,border:'1px solid #e5e7eb'}}><div style={{fontWeight:900,fontSize:18}}>{g.title}{g.subtype?` (${g.subtype})`:''}</div><div style={{marginTop:6}}>Paid by <b style={{color:'#15803d'}}>{g.paidby}</b> • Total ${g.amount.toFixed(2)} • Split by {g.participants.length}</div>{g.owes.length?g.owes.map((o,k)=><div key={k} style={{display:'flex',justifyContent:'space-between',padding:'10px 0',borderTop:'1px solid #e5e7eb',marginTop:8}}><span>{o.name} ➜ {g.paidby}</span><div style={{display:'flex',gap:8,alignItems:'center'}}><span style={{fontWeight:800}}>${o.amount.toFixed(2)}</span><input style={{...input,minWidth:90,padding:8}} placeholder='Paid $' value={(settlementRows.find(r=>r.row_key===`${i}-${k}`)?.paid_amount ?? settlementAmounts[`${i}-${k}`] ?? '')} onChange={async e=>{const val=e.target.value;setSettlementAmounts({...settlementAmounts,[`${i}-${k}`]:val});await supabase.from(TABLES.settlements).upsert({row_key:`${i}-${k}`,paid_amount:Number(val||0),paid:false,title:g.title,payer:g.paidby,debtor:o.name},{onConflict:'row_key'});}}/><span style={{fontSize:13}}>Bal: ${Math.max(0,o.amount-Number((settlementRows.find(r=>r.row_key===`${i}-${k}`)?.paid_amount ?? settlementAmounts[`${i}-${k}`] ?? 0))).toFixed(2)}</span><button style={{...btn,padding:'8px 12px',minHeight:36,borderRadius:12,fontSize:13}} onClick={async()=>{setSettlementPaid({...settlementPaid,[`${i}-${k}`]:true});await supabase.from(TABLES.settlements).upsert({row_key:`${i}-${k}`,paid:true,paid_amount:Number(settlementAmounts[`${i}-${k}`]||o.amount),title:g.title,payer:g.paidby,debtor:o.name},{onConflict:'row_key'});}}>{(settlementRows.find(r=>r.row_key===`${i}-${k}`)?.paid || settlementPaid[`${i}-${k}`])?'Settled ✅':'Mark Paid'}</button><button style={{...btn,padding:'8px 10px',minHeight:36,borderRadius:12,fontSize:12,background:'linear-gradient(135deg,#f59e0b,#ea580c)'}} onClick={()=>{setSettlementAmounts({...settlementAmounts,[`${i}-${k}`]:''});setSettlementPaid({...settlementPaid,[`${i}-${k}`]:false});}}>Edit</button><button style={{...btn,padding:'8px 10px',minHeight:36,borderRadius:12,fontSize:12,background:'linear-gradient(135deg,#dc2626,#7f1d1d)'}} onClick={()=>setSettlementPaid({...settlementPaid,[`${i}-${k}`]:false})}>Delete</button></div></div>):<div style={{marginTop:10,color:'#15803d'}}>No one owes. Solo expense.</div>}</div>)}</div><div style={{marginTop:14,padding:14,background:'#eff6ff',borderRadius:16}}>✅ Accountant Mode Active — Stay, Activities, Food, Fuel, Other are kept separate. Track partial payments, balances, mark settled, edit resets, or delete mistakes.</div></div>}
...
{tab==='tracker' && <div style={card}><h2>🛰️ Live Map Tracker</h2><p>{location.status}</p><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><select style={input} value={trackName} onChange={e=>setTrackName(e.target.value)}><option value=''>Traveler</option>{members.map((m,i)=><option key={i} value={m.name}>{m.name}</option>)}</select><button style={btn} onClick={addLiveTracker}>Use My Current Location</button><button style={{...btn,background:'linear-gradient(135deg,#16a34a,#059669)'}} onClick={()=>setTrackingOn(!trackingOn)}>{trackingOn?'🛑 Stop Live Tracking':'🟢 Start Live Tracking'}</button></div><div style={{marginTop:16,padding:16,background:'#eff6ff',borderRadius:16}}><div style={{fontWeight:800,marginBottom:8}}>🗺️ Live Map V4</div><a href='https://www.google.com/maps/search/Red+River+Gorge+Kentucky' target='_blank'>🗺️ Open Full Map</a><div style={{marginTop:8,fontSize:14}}>See each traveler below with live ETA, distance, and status.</div></div><div style={{display:'grid',gap:12,marginTop:16}}>{liveTrackers.map((t,i)=>{const me=(location.lat&&location.lng)?{lat:Number(location.lat),lng:Number(location.lng)}:null;const tl=Number(t.lat),tg=Number(t.lng);const dist=me?Math.sqrt(Math.pow((me.lat-tl)*69,2)+Math.pow((me.lng-tg)*54.6,2)):null;const mins=dist?Math.max(1,Math.round(dist*2)):null;const online=t.updated_at?(Date.now()-new Date(t.updated_at).getTime()<60000):false;return <div key={i} style={{padding:16,background:'#fff',borderRadius:18,border:'1px solid #e5e7eb'}}><b>{t.name}</b><p style={{margin:'6px 0'}}>📍 {online?'Online':'Idle'} • {t.status}</p><p>Lat {t.lat} • Lng {t.lng}</p><p style={{fontSize:12,color:'#64748b'}}>Updated: {t.updated_at?new Date(t.updated_at).toLocaleTimeString():''}</p><p style={{fontSize:13}}>{dist?`📏 ${dist.toFixed(1)} mi away • ETA ${mins} min`:''}</p><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><a href={`https://www.google.com/maps?q=${t.lat},${t.lng}`} target='_blank'>Open Pin</a><a href={`https://www.google.com/maps/dir/${location.lat||''},${location.lng||''}/${t.lat},${t.lng}`} target='_blank'>Route</a><button onClick={()=>removeLiveTracker(i)}>Delete</button></div></div>})}</div></div>}

{tab==='comments' && <div style={card}><h2>💬 Comments PRO</h2><div style={{marginBottom:10,padding:10,background:'#ecfeff',borderRadius:14}}>{Object.keys(typingUsers).length?Object.keys(typingUsers).join(', ')+' typing...':'Live chat synced for everyone'}</div><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><select style={input} value={commentName} onChange={e=>setCommentName(e.target.value)}><option value=''>Choose traveler</option>{members.map((m,i)=><option key={i} value={m.name}>{m.name}</option>)}</select><input style={input} placeholder='Write comment' value={commentText} onChange={e=>{setCommentText(e.target.value); if(commentName) setTypingUsers({...typingUsers,[commentName]:true});}}/><button style={btn} onClick={addComment}>{editingComment!==null?'💾 Save':'📨 Send'}</button></div>{replyTo!==null?<div style={{marginTop:8,fontSize:13}}>Replying to <b>{comments[replyTo]?.name}</b> <button onClick={()=>setReplyTo(null)}>Cancel</button></div>:null}<div style={{display:'grid',gap:10,marginTop:14}}>{comments.map((c,i)=><div key={i} style={{padding:14,background:'#fff',borderRadius:18,border:'1px solid #e5e7eb'}}><b>{c.name}</b>{c.reply_to?<div style={{fontSize:12,color:'#64748b'}}>↪ Reply</div>:null}<p style={{margin:'8px 0'}}>{c.text}</p><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><button onClick={()=>{setReplyTo(i);setCommentName(commentName||'');}}>Reply</button><button onClick={()=>{setEditingComment(i);setCommentName(c.name);setCommentText(c.text);}}>Edit</button><button onClick={()=>removeComment(i)}>Delete</button></div></div>)}</div></div>}

{tab==='guide' && <div style={card}><h2>🗺️ Travel Dashboard</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:16}}><div style={{padding:16,background:'#fff',borderRadius:16,border:'1px solid #e5e7eb'}}><h3>📍 Beautiful Spots</h3><p>Sky Bridge 🌄</p><p>Natural Bridge 🪨</p><p>Chimney Top Rock 📸</p><p>Princess Arch 🥾</p></div><div style={{padding:16,background:'#fff',borderRadius:16,border:'1px solid #e5e7eb'}}><h3>🧭 Live GPS Maps</h3><p>{location.status}</p><p><a href={location.lat ? `https://www.google.com/maps/dir/${location.lat},${location.lng}/Red+River+Gorge+Kentucky` : 'https://maps.google.com/?q=Red+River+Gorge+Kentucky'} target='_blank'>🚗 Navigate to Red River Gorge</a></p><p><a href='https://maps.google.com/?q=Sky+Bridge+Kentucky' target='_blank'>🌄 Drive to Sky Bridge</a></p><p><a href='https://maps.google.com/?q=Natural+Bridge+Kentucky' target='_blank'>🪨 Drive to Natural Bridge</a></p><p><a href='https://maps.google.com/?q=gas+station+near+Red+River+Gorge' target='_blank'>⛽ Nearby Gas</a></p></div><div style={{padding:16,background:'#fff',borderRadius:16,border:'1px solid #e5e7eb'}}><h3>🌦️ Live Weather</h3><p>Location: Slade, KY</p><p>{weather.temp} • {weather.desc}</p><p>Humidity {weather.humidity} • Wind {weather.wind}</p></div><div style={{padding:16,background:'#fff',borderRadius:16,border:'1px solid #e5e7eb'}}><h3>📰 Local News & Alerts</h3><p>{location.lat ? 'Using your live location for nearby updates' : 'Enable GPS for nearby updates'}</p><p><a href={location.lat ? `https://www.google.com/search?q=news+near+${location.lat},${location.lng}` : 'https://www.google.com/search?q=Red+River+Gorge+Kentucky+news'} target='_blank'>📍 Nearby News</a></p><p><a href='https://www.google.com/search?q=Kentucky+traffic+alerts' target='_blank'>🚗 Traffic Alerts</a></p><p><a href='https://www.google.com/search?q=Red+River+Gorge+weather+warning' target='_blank'>⛈ Weather Warnings</a></p></div><div style={{padding:16,background:'#fff',borderRadius:16,border:'1px solid #e5e7eb'}}><h3>📸 Luxury Experiences</h3><p>Sunrise photography session</p><p>Cabin hot tub night</p><p>Scenic helicopter inquiry</p></div></div></div>}

{tab==='assistant' && <div style={card}><h2>🤖 AI ChatGPT Assistant PRO</h2><div style={{marginBottom:12,padding:14,background:'#ecfeff',borderRadius:16,fontWeight:700}}>Unlimited smart assistant for travel, maps, science, finance, jokes, local help, route planning, and universal questions.</div><button style={btn} onClick={startVoice}>{listening ? '🎤 Listening...' : '🎤 Start Voice Command'}</button><button style={{...btn,marginLeft:10,background:'linear-gradient(135deg,#16a34a,#059669)'}} onClick={()=>{const msg='Driving mode activated. Live alerts enabled.'; setAiTip('🚗 Driving Mode ON: voice alerts, traffic warnings, fuel stops, weather alerts.'); if('speechSynthesis' in window){speechSynthesis.cancel(); speechSynthesis.speak(new SpeechSynthesisUtterance(msg));}}}>🚗 Driving Mode</button><button style={{...btn,marginLeft:10,background:'linear-gradient(135deg,#ea580c,#f59e0b)'}} onClick={()=>{const area=location.lat?`${Number(location.lat).toFixed(2)}, ${Number(location.lng).toFixed(2)}`:'your route'; const msg=`Smart road mode enabled. Monitoring ${area}.`; setAiTip(`🛣️ Auto Road Mode ON: scanning near ${area} for gas, food, traffic, rain, police alerts, and best stops ahead.`); if('speechSynthesis' in window){speechSynthesis.cancel(); speechSynthesis.speak(new SpeechSynthesisUtterance(msg));} window.open(location.lat?`https://www.google.com/search?q=gas+food+traffic+near+${location.lat},${location.lng}`:'https://www.google.com/search?q=road+trip+gas+food+traffic+near+me','_blank');}}>🛣️ Auto Road Mode</button><button style={{...btn,marginLeft:10,background:'linear-gradient(135deg,#0ea5e9,#2563eb)'}} onClick={()=>{if(location.lat){window.open(`https://www.google.com/maps/dir/${location.lat},${location.lng}/Red+River+Gorge+Kentucky`,'_blank');setAiTip('🗺️ GPS Navigation started from your live location.');}else{navigator.geolocation.getCurrentPosition((pos)=>{window.open(`https://www.google.com/maps/dir/${pos.coords.latitude},${pos.coords.longitude}/Red+River+Gorge+Kentucky`,'_blank');setAiTip('🗺️ GPS Navigation started from your location.');},()=>setAiTip('Enable GPS to start maps navigation.'));}}}>🗺️ Start GPS Map</button><p style={{marginTop:14}}>{aiTip}</p><div style={{marginTop:12,padding:16,background:'#eff6ff',borderRadius:16}}><p>📍 Best Photo Spot Now: Sky Bridge at sunset</p><p>🍔 Food Stop: Miguel's Pizza nearby</p><p>🌧 Rain Alert: Check weather before hiking</p><p>⛽ Gas Tip: Fill up in Slade before trails</p><p>🎙 Try saying: weather / budget / photo spot / take me there</p><p>🚗 Driving Mode speaks alerts aloud while passenger drives.</p><p>🛣️ Auto Road Mode scans ahead for gas, food, traffic, storms, and stop ideas.</p></div></div>}

{tab==='emergency' && <div style={card}><h2>🚨 Emergency Contacts</h2><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><input style={input} placeholder='Choose traveler' value={emName} onChange={e=>setEmName(e.target.value)}/><input style={input} placeholder='Phone' value={emPhone} onChange={e=>setEmPhone(e.target.value)}/><select style={input} value={emType} onChange={e=>setEmType(e.target.value)}><option>Friend</option><option>Family</option><option>Hospital</option><option>Police</option><option>Roadside</option></select><button style={btn} onClick={addEmergency}>Add</button></div>{emergencyContacts.map((c,i)=><p key={i}><b>{c.name}</b> • {c.type} • 📞 <a href={`tel:${c.phone}`}>{c.phone}</a> <button onClick={()=>removeEmergency(i)}>Delete</button></p>)}<div style={{marginTop:14,padding:14,background:'#fee2e2',borderRadius:16}}><p>📍 In emergency use Guide tab for GPS route.</p><p>🏥 Nearest hospital listed above.</p></div></div>}

<div style={{position:'fixed',bottom:10,left:10,right:10,backdropFilter:'blur(28px)',WebkitBackdropFilter:'blur(28px)',background:'rgba(15,23,42,0.68)',border:'1px solid rgba(255,255,255,0.14)',display:'flex',justifyContent:'space-around',padding:'14px 10px',zIndex:50,borderRadius:28,boxShadow:'0 24px 50px rgba(0,0,0,.28)'}}>{[['people','👥'],['tracker','📍'],['comments','💬'],['assistant','🤖'],['guide','🗺️']].map(([k,icon])=><button key={k} onClick={()=>setTab(k)} style={{background:'transparent',border:0,color:'#fff',fontSize:24,opacity:tab===k?1:0.6,transform:tab===k?'scale(1.15)':'scale(1)',transition:'all .2s ease'}}>{icon}</button>)}</div><div style={{marginTop:40,padding:'10px 0',textAlign:'center',color:'#e2e8f0',fontSize:13,fontWeight:700}}>Built by Vikesh • @vikesh-2026</div>
</div>
}
