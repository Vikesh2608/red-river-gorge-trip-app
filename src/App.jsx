...
const startVoice=()=>{
 ...
 const smartReply=(q)=>{
   ...
   return `🌐 Universal Assistant: ${place}. Ask me any topic—travel, maps, people tracker, finance, news, history, science, sports, jokes, or directions.`;
 };
...
{tab==='public' && <div style={card}><h2>🌍 Social Travel Lounge</h2><div style={{marginBottom:14}}><a href='https://chat.whatsapp.com/I9s6EZK81X05llarQSPyDs' target='_blank' style={{...btn,textDecoration:'none',display:'inline-block'}}>💬 Join Trip WhatsApp Group</a><button style={btn} onClick={()=>(()=>{const msg=location.lat?`📍 My Live GPS: https://maps.google.com/?q=${location.lat},${location.lng}`:'📍 Share your live location from WhatsApp: Attach > Location > Live Location';window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`,'_blank');})()}>📍 WhatsApp GPS Share</button></div><p>Members: {members.length} • Budget ${totalBudget} • Split ${split}</p><div style={{display:'flex',gap:10,flexWrap:'wrap',margin:'14px 0'}}><input style={input} placeholder='Your name' value={postName} onChange={e=>setPostName(e.target.value)}/><input style={input} placeholder='Share update...' value={postText} onChange={e=>setPostText(e.target.value)}/><button style={btn} onClick={addPost}>Post</button></div><div style={{display:'grid',gap:12}}>{publicPosts.map((p,i)=><div key={i} style={{padding:16,background:'#fff',borderRadius:18,border:'1px solid #e5e7eb'}}><b>{p.name}</b><p style={{margin:'8px 0'}}>{p.text}</p><div style={{display:'flex',gap:10}}><button onClick={()=>likePost(i)}>❤️ {p.likes}</button><button onClick={()=>removePost(i)}>Delete</button></div></div>)}</div><div style={{marginTop:16,padding:16,background:'#eff6ff',borderRadius:16}}>Live social feed for trip members. Share plans, ETA, cabin updates, hiking ideas, photos, and meetups. Use 📍 Share Live Location in WhatsApp during travel day.</div></div>}
