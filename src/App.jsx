...
{tab==='settlement' && <div style={card}><h2>💸 Settlement Tracker</h2><p>Mark payments complete after Zelle/Cash/Venmo.</p><div style={{display:'grid',gap:12}}>{members.map((m,i)=>{const due=Math.max(0,-(balances[m.name]||0));const get=Math.max(0,(balances[m.name]||0));const paid=due<0.01;return <div key={i} style={{padding:14,background:'#fff',borderRadius:16,border:'1px solid #e5e7eb',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><b>{m.name}</b><div style={{fontSize:14,color:paid?'#15803d':'#b91c1c'}}>{paid?'Settled ✅':due?`Owes $${due.toFixed(2)}`:`Gets Back $${get.toFixed(2)}`}</div></div><div style={{fontSize:22}}>{paid?'🟢':'🔴'}</div></div>})}</div></div>}
